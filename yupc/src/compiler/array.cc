...
Instruction *ca::create_array_dyn_malloc(Type *elem_type, Value *elem_count, DataLayout dl) {
    Type *i32 = Type::getInt32Ty(*com_un::comp_units.top()->context);

    Constant *size_of_element = ConstantInt::get(i32, dl.getTypeAllocSize(elem_type));

    //Constant *length = ConstantInt::get(i32, elem_count);
    Value *array_size = com_un::comp_units.top()->ir_builder.CreateMul(size_of_element, elem_count);

    Instruction *array_malloc = CallInst::CreateMalloc(
        com_un::comp_units.top()->ir_builder.GetInsertBlock(), 
        elem_type, 
        elem_type, array_size,
        nullptr, nullptr, ""); 

    com_un::comp_units.top()->ir_builder.Insert(array_malloc);
...
void ca::indexed_access_expr_codegen(Value *array, Value *idxVal) {
    ...
    Type *ptr_type = array->getType()->getPointerElementType();

    Value *idx_gep = com_un::comp_units.top()->ir_builder.CreateGEP(ptr_type, array, idxVal);

    com_un::comp_units.top()->value_stack.push(idx_gep);
}
...
void ca::arr_elem_assignment_codegen(std::string arr_name, 
    ...
    AllocaInst *stored = com_un::comp_units.top()->symbol_table.top()[arr_name];

    Type *alloc_type = stored->getAllocatedType();
    LoadInst *array = com_un::comp_units.top()->ir_builder.CreateLoad(alloc_type, stored);

    Value *val_to_override;
    for (size_t i = 0; i < idx_nesting_lvl; i++) { 

        Type *arr_ptr_type = array->getType()->getPointerElementType();

        Value *idx_gep = com_un::comp_units.top()->ir_builder.CreateGEP(arr_ptr_type, array, idx_vals[i]);
    ...
    com_un::comp_units.top()->ir_builder.CreateStore(val, val_to_override, false);
...
void ca::array_codegen(std::vector<Value*> elems, size_t elem_count) {
    ...
    DataLayout dl = com_un::comp_units.top()->module->getDataLayout();

    Instruction *array_malloc = ca::create_array_const_malloc(elem_type, elem_count, dl);

    for (size_t i = 0; i < elem_count; i++) {
        Constant *idx = ConstantInt::get(Type::getInt32Ty(*com_un::comp_units.top()->context), i);

        Type *alloc_ptr_type = array_malloc->getType()->getPointerElementType();

        Value *idx_gep = com_un::comp_units.top()->ir_builder.CreateGEP(alloc_ptr_type, array_malloc, idx);

        com_un::comp_units.top()->ir_builder.CreateStore(elems[i], idx_gep);
    }

    com_un::comp_units.top()->value_stack.push(array_malloc);
}

std::any cv::Visitor::visitIndexedAccessExpr(parser::YupParser::IndexedAccessExprContext *ctx) {

    this->visit(ctx->expr(0));
    Value *array = com_un::comp_units.top()->value_stack.top();

    this->visit(ctx->expr(1));
    Value *idx_val = com_un::comp_units.top()->value_stack.top();
...
std::any cv::Visitor::visitArr_elem_assignment(parser::YupParser::Arr_elem_assignmentContext *ctx) {
    ...
    for (size_t i = 0; i < idx_nesting_lvl; i++) {
        ...
        idx_vals.push_back(com_un::comp_units.top()->value_stack.top());
    }
...
std::any cv::Visitor::visitArray(parser::YupParser::ArrayContext *ctx) {
    ...
    for (size_t i = 0; i < elem_count; i++) {
        ...
        Value *elem = com_un::comp_units.top()->value_stack.top();
...
std::any cv::Visitor::visitArray_init(parser::YupParser::Array_initContext *ctx) {
    ...
    this->visit(ctx->expr());
    Value *array_size = com_un::comp_units.top()->value_stack.top();
